...
export default function HeroAnimation() {
  ...
  useEffect(() => {
    ...
    const canvasRadius = Math.sqrt(Math.pow(canvas.width, 2) + Math.pow(canvas.height, 2)) / 2;
    const startDistance = canvasRadius * 0.9; // Start from near the edge

    // Function to create asteroid-like particles
    function createAsteroidParticle(distance, startDist) {
    ...
    for (let i = 0; i < streamCount; i++) {
      ...
      for (let j = 0; j < 6; j++) { // Fewer initial particles (was 12)
        const distance = length * (0.3 + Math.random() * 0.7);
        streams[i].particles.push(createAsteroidParticle(distance, startDistance));
      }
    }
    ...
    function drawAsteroidParticle(ctx, x, y, particle, opacity) {
      ctx.save();
      ctx.translate(x, y);
    ...
    const animate = (time: number) => {
      ...
      gradient.addColorStop(0.5, `rgba(80, 70, 200, ${sphere.brightness * 0.8})`);
      ...
      for (const ring of rings) {
        ...
        for (let i = 0; i <= ring.segments; i++) {
          ...
          const waveOffset = Math.sin(angle * ring.waveFrequency + ring.wavePhase) * ring.waveAmplitude;
      ...
      for (const point of sphere.wireframe) {
        ...
        const x = point.x * Math.cos(time * rotationSpeed) - point.z * Math.sin(time * rotationSpeed);
        const z = point.x * Math.sin(time * rotationSpeed) + point.z * Math.cos(time * rotationSpeed);
      ...
      for (const particle of sphere.particles) {
        ...
        const y = sphere.y + Math.sin(particle.angle) * particle.distance + particle.yOffset;
        ...
        if (particle.size > 1.5) {
          ...
          glow.addColorStop(0, `rgba(150, 150, 255, ${particle.opacity * 0.4})`);
      ...
      for (const stream of streams) {
        ...
        for (let i = stream.particles.length - 1; i >= 0; i--) {
          ...
          const curveIntensity = Math.sin(progress * Math.PI) * particle.curveStrength;
          ...
          const waveEffect = Math.sin(progress * 10 + wavePhase) * 
          ...
          const normalizedDistance = Math.max(0, Math.min(1, (distanceToCenter - sphere.radius) / fadeDistance));
          const adjustedOpacity = particle.opacity * (1 - normalizedDistance * 0.3);
          ...
          trailGradient.addColorStop(0, `rgba(180, 180, 255, ${adjustedOpacity * particle.trailOpacity})`);
